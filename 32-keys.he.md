...
ולכן ההבדל **הראשון** שנשאר הוא לא Bluetooth אלא מה שנשאר עומד לצד המקום שבו הוא היה: שלושה מקשים בשכבה 5 — `&studio_unlock` על `z`, `&soft_off` על `g` ו-`&bootloader` על `b` — ממוקמים כך שקומבו הכניסה לשכבה, `x`+`c`+`v`, לא נוגע באף אחד מהם. השלושה נשארים כי ל-ZMK יש אותם ול-QMK אין מקבילה שאליה אפשר להעביר אותם — זה הבדל ביכולת, לא שארית. כל השאר בחצי הזה חזר להיות `&trans`, והשכבה שינתה שם מ-`bt` ל-`fkeys`, כי זה כל מה שהיא עכשיו. **שכבה 5 ירדה מחמישה־עשר מיקומים חורגים לשלושה,** וזה רווח הזהות הגדול ביותר מאז ההעברה עצמה, והוא בא ממחיקת מקשים ולא ממצוא בתים להם. ההבדל **השני** הוא ההגנה על מקשי השכבה ב-`&lt`, שאין ל-QMK בורג מקביל עבורה, ולכן היא נשארת שיפור של ה-Corne בלבד. ה**שלישי** הוא האגודל השמאלי, שתואר למעלה: Shift רגיל, כי ZMK לא יכול לבטא הקשה-ואז-החזקה ל-Caps Lock והתחליף המובן מאליו יורה בטעות.
...
| 4 — פונקציות | `TO(0)` | `DF(0)`, `&to 0` ב-Corne | Caps Lock |
...
**`LCTL(GRAVE)`** — Ctrl ועוד גרש הפוך, צירוף מחזור הטרמינלים — ו-**Caps Lock** לא הוכרעו, ושניהם בתדירות נמוכה מספיק כדי שיהיה סביר לזרוק אותם ולא להעביר. התחליף הקרוב של Caps Lock, `caps_word`, יושב עכשיו על האגודל הפנימי השמאלי של שכבה 4 בשתי המקלדות, שזה בתוך הליבה ובתוך 36 המקשים — ולכן לזרוק את Caps Lock עולה פחות ממה שזה עלה כשזה נכתב. לצירוף הטרמינלים עדיין אין תחליף בכלל.
...
13. **האם ה-dance של Caps Lock על האגודל השמאלי שורד?** ה-Ximi2 מקישה על האגודל הזה ל-Shift ומקישה-ואז-מחזיקה אותו ל-Caps Lock; ל-ZMK אין סלוט של הקשה-ואז-החזקה, ולכן ב-Corne יש Shift רגיל ו-Caps Lock נלקח משכבה 4. זה השיוך היחיד שההעברה לא הצליחה לשקף. התיקון הזול הוא לזרוק את ה-dance ב-Ximi2 ולתת לשתי המקלדות לקחת את Caps Lock משכבה 4 — וזה גם עוד זמן הקשה של tap dance שנפרש, והתבנית אומרת שאלה מעולם לא חסרו.

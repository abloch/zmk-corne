...
החצי השמאלי הוא הקשה, ושני שלישים ממנו חיים בשתי המקלדות. Tab ו-Escape שניהם בתדירות גבוהה, שניהם עדיין על העמודה החיצונית, ולאף אחד מהם אין עדיין גזר דין שהוחלט עליו — אף על פי של-Escape יש עכשיו קומבו, וזה חדש מאז שזה נכתב ומכוסה בשלב ארבע. העמדה התחתונה מתה בשתי המקלדות: שם ישב הגרש ההפוך, ואותו מהלך ששחרר אותה ב-Ximi2 שחרר אותה ב-Corne.
...
Caps Lock הוא המקום שבו הפער נראה, והוא נסגר. כשהוא עבר לאגודל של ה-Ximi2 הוא פינה את העמודה החיצונית השמאלית של שכבה 4, והמקש נמחק בשתי המקלדות — מה שהשאיר את ה-Corne בלי שום שיוך של Caps Lock, כי ZMK לא יכול לבטא את המחווה שהחליפה אותו. ה-Ximi2 מאז החזירה את המקש, לצד המחווה ולא במקומה, וה-Corne משייכת את אותה עמדה. שתי דרכים במקלדת אחת, דרך אחת בשנייה, ושום דבר לא בלתי נגיש באף אחת מהן. המאקרו `capslock` משויך ולא חונה, ומספר ההבדלים המכוונים בין שתי המפות ירד משלושה לשניים.
...
- ~~**מאקרו יתומים `M11`, `M21`, `M25`, `M26`.**~~ נסגר; הסלוטים האלה ריקים בייצוא הנוכחי. הכפילות שנשארה היא `M28` ו-`M29`, עותקים מדויקים של `M18` ו-`M19` — חצי ה-`⌘⌥` למעלה ולמטה. `M11` כבר לא יתום בכלל — הוא Caps Lock שוב, משויך על המקש התחתון של העמודה החיצונית השמאלית בשכבה 4 בשתי המקלדות. שני הזוגות חיים על אשכול התוספת, שנשאר, ולכן הכפילות הזאת היא עכשיו תכונה של אשכול בונוס ולא פגם שמחכה לשלב.
...
| 2 — ניווט | `TO(0)` | `␛` | `LCTL(GRAVE)` — ב-Corne בלבד; ה-Ximi2 ניקתה אותו |
| 3 — ספרות | `TO(0)` | `DF(0)`, `&to 0` ב-Corne | מת |
| 4 — פונקציות | `TO(0)` | `DF(0)`, `&to 0` ב-Corne | Caps Lock — חזר, ובשתי המקלדות |
...
**Caps Lock** הוכרע, והוא הוכרע הפוך ממה שהחלק הזה ניבא. התחזית הייתה: הועבר במקלדת אחת, נזרק בשנייה. מה שקרה הוא שה-Ximi2 לקחה אותו להקשה-ואז-החזקה של האגודל השמאלי *ואז החזירה את מקש שכבה 4 לצידו*, ולכן המחווה היא קיצור דרך ולא הדרך היחידה — וה-Corne, שלא יכולה לבטא את המחווה, משייכת את המקש. שתי המקלדות שומרות גם את `caps_word` על האגודל הפנימי השמאלי של שכבה 4, בתוך הליבה. המחיר הוא ש-Caps Lock הוא **דייר של העמודה החיצונית השמאלית שוב**, בשכבה 4, וצריך למצוא לו בית יחד עם השאר במקום שיהיה דבר אחד פחות לחשוב עליו. העמדה התחתונה מתה בשכבות 0 ו-3 בלבד.
...
5. **מה קורה ל-`LCTL(GRAVE)`?** בתדירות נמוכה מספיק כדי לזרוק ולא להעביר, ואין לו תחליף. הוא גם חריגה עכשיו ולא דייר משותף: ה-Ximi2 ניקתה את העמדה הזאת וה-Corne עדיין משייכת אותה, מה שהופך זריקה ב-Corne לדרך הזולה ביותר לסגור את הפער — אלא אם הרגל מחזור הטרמינלים מתברר כאמיתי, ואז הוא צריך בית בליבה וה-Ximi2 צריכה אותו בחזרה. Caps Lock, שהשאלה הזו הייתה חצי עליו, סגור, והוא חזר על המקש התחתון של העמודה החיצונית השמאלית בשכבה 4 בשתי המקלדות.
...
| 4 — בתים לעמודה השמאלית החיצונית | חשיבה, בעיקר | Escape נענה, Tab, Caps Lock והשאר פתוחים | אותו דבר, בשתי המקלדות בבת אחת |
| 5 — פרישת העמודה השמאלית | המבחן האמיתי. שבועיים | לא התחיל | לא התחיל |

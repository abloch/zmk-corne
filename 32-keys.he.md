...
11. **למה האגודל השמאלי בשכבת הבסיס הוא Cmd הימני?** `KC_RGUI` ב-Ximi2, `&kp RGUI` ב-Corne, בזמן שכל שכבה אחרת בשתי המקלדות משתמשת ב-Cmd השמאלי. זה הועתק בנאמנות ולא תוקן בשקט, שזה האינסטינקט הנכון בתוך העברה ואינו תשובה. או שיש סיבה שאף אחד לא כתב, או שזו פליטה שנושאת משקל מספיק זמן כדי להרגיש מכוונת. תכריע, ואז תיישר את כל שתים־עשרה העמדות.
12. **האם ה-dance של Caps Lock על האגודל השמאלי שורד?** ה-Ximi2 מקישה על האגודל הזה ל-Shift ומקישה-ואז-מחזיקה אותו ל-Caps Lock; ל-ZMK אין סלוט של הקשה-ואז-החזקה, ולכן ב-Corne יש Shift רגיל ו-Caps Lock נלקח משכבה 4. זה השיוך היחיד שההעברה לא הצליחה לשקף. התיקון הזול הוא לזרוק את ה-dance ב-Ximi2 ולתת לשתי המקלדות לקחת את Caps Lock משכבה 4 — וזה גם עוד זמן הקשה של tap dance שנפרש, והתבנית אומרת שאלה מעולם לא חסרו.
...
| 2 — הבאת ה-Corne לזהות | ישיבה אחת של עבודת קונפיג | מקור האמת | **בוצע** |

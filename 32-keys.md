# Going Small

### One 36-key layout, two keyboards, and hardware bought only once both of them already run it

---

This is no longer a plan. It is a status report with a plan attached to the end of it.

On the **Ximi2**, the work keyboard, the right outer column is retired on every layer. Mouse emulation is gone from the keys — no movement, no scroll, nothing but the four pointer combos that survive because no trackpad maps them. Layer access was torn out and rebuilt as a uniform momentary/locked pair per layer, the same shape on every layer, learnable in an evening. Backtick found a permanent home on the right pinky bottom, which retired tilde in the same stroke. Two Vial bugs were found, understood and fixed. The left outer column's middle and bottom keys are both dead on every layer, and the top key has since gone the same way on the base layer and on layer 1 — Tab moved to the right outer thumb as `LALT_T(KC_TAB)`, hold for Alt and tap for Tab. What is left on that column is a return-to-base copy on layers 2 to 5 that position 5 already carries.

On the **Corne**, the home keyboard, all of that has now landed, in one sitting. `config/corne.keymap` is a position-for-position port of `akiva.vil`: every binding on all six layers matches the Ximi2, and the only things left behind are the ones the Corne does not physically have — the outer seventh column, the extra per-half cluster, the rotary encoders, the attached mouse.

That gap was the single most important fact in this document, and closing it was never a filing detail to be cleaned up at the end. **Convergence is the goal, not a chore on the way to it.** The endgame is one 36-key layout that runs identically on both boards — and possibly on two new pieces of hardware, since the work board needs a 36-key answer of its own: either a new Ximi, or simply pulling the outer switches off the one you have. Two boards, one layout, one set of hands.

Which is why the Corne was never trailing scenery. It is half the target, and every evening spent on an unported Corne was practice on the layout you are trying to leave. That is why porting it jumped the queue instead of closing it, and it is why the rule about never advancing on one board alone is back in force from here. What is left to decide is the left outer column, and it is now the same decision on both boards at the same time.

---

## The destination

Thirty keys plus six thumbs — three rows of five columns per half, three thumb keys per half, columnar stagger. That is the beekeeb Toucan2's 36-key build, a Cantor/Piantor shape, and it is precisely your Corne minus the two outer pinky columns and your Ximi2 minus those same columns plus its extra cluster. All six thumb keys survive on every board.

```mermaid
%%{init: {'theme':'base','themeVariables':{'primaryColor':'#eef1f5','primaryTextColor':'#0b0f14','primaryBorderColor':'#5b6673','nodeTextColor':'#0b0f14','textColor':'#0b0f14','mainBkg':'#eef1f5','fontSize':'18px'}}}%%
block
columns 11

  q w e r t space y u i o p
  a s d f g space h j k l bspc["⌫"]
  z x c v b space n m cma[","] dot["."] grv[" `"]
  space:2 cmd["⌘"] spc["␣"] sft["⇧"] space ctl["⌃"] ent["⏎"] alt["⌥<br/>⇥"] space:2

  classDef core fill:#eef1f5,stroke:#5b6673,stroke-width:2px,color:#0b0f14
  classDef thumb fill:#c8ece0,stroke:#0b5946,stroke-width:3px,color:#06241c

  class q,w,e,r,t,a,s,d,f,g,z,x,c,v,b,y,u,i,o,p,h,j,k,l,bspc,n,m,cma,dot,grv core
  class cmd,spc,sft,ctl,ent,alt thumb
```

One key in there has already changed identity. The right pinky's bottom position carries backtick now rather than slash, and that swap is one of the better decisions in the whole migration — the reasoning is under *Swap a redundant key for an orphan* below.

This layout is the destination for **both** boards. Hardware is the last question, not the first: a Toucan2 for home, and for work either a 36-key Ximi if one exists or the current one with its outer switches pulled. Either way you will already be typing this.

### What is still on the block

Both right halves are now finished apart from one tenant, and it is the same tenant on each. Lock screen still sits on the outer column's top key; the other two positions are dead on all six layers, on both boards. The right outer *thumb* has gained a second job in the same pass — it is Tab's new home — but that is a thumb, and all six thumbs survive to the Toucan2.

The Corne's copy of that column used to carry browser forward, browser back and tilde. All three are gone: forward and back rolled inward onto `i`+`p` and `k`+`⌫`, and tilde left with the backtick move, since tilde is nothing but shifted backtick. What the plan asked for was that column dead on every layer, and that is not quite what happened — `m10` survives on it, because the lock screen has no home inside the core yet and inventing a bad one to satisfy a diagram would be the wrong trade. It is a named deferral on both boards rather than an oversight on either.

The left half was the hard one and it is down to a single live position on both boards. The bottom key went first: backtick used to live there, and the same move that freed it on the Ximi2 freed it on the Corne, and what briefly moved back in — `?`, Ctrl-backtick, Caps Lock — has since been dropped or rehoused. The middle key went with this pass: Escape is a combo now, `q`+`e`, with no key beside it. That left the top key, Tab on the base layer and a return-to-base copy on the five others. Both halves of it are now answered. Tab went to the right outer thumb as a hold-tap: hold for Alt, tap for Tab, a single press on the base layer with no combo and no stretch. And layer 1's copy went with it, because layer 1 turned out to be reachable only by holding its thumb — no `TO(1)`, `MO(1)`, `TG(1)` or combo entry exists on either board — so nothing can strand you there and the escape hatch was never load-bearing. Layers 2 to 5 still carry the copy, redundantly, beside the one on position 5.

```mermaid
%%{init: {'theme':'base','themeVariables':{'primaryColor':'#eef1f5','primaryTextColor':'#0b0f14','primaryBorderColor':'#5b6673','nodeTextColor':'#0b0f14','textColor':'#0b0f14','mainBkg':'#eef1f5','fontSize':'18px'}}}%%
block
columns 13

  xl1["❌"] q w e r t space y u i o p lock["🔒"]
  xl2["❌"] a s d f g space h j k l bspc["⌫"] xr1["❌"]
  xl3["❌"] z x c v b space n m cma[","] dot["."] grv[" `"] xr2["❌"]
  space:3 cmd["⌘"] spc["␣"] sft["⇧"] space ctl["⌃"] ent["⏎"] alt["⌥<br/>⇥"] space:3

  classDef doomed fill:#ffd5d0,stroke:#8c1008,stroke-width:3px,color:#3d0603
  classDef dead fill:#f2f3f5,stroke:#98a2ae,stroke-width:2px,stroke-dasharray:5 4,color:#5b6673
  classDef core fill:#eef1f5,stroke:#5b6673,stroke-width:2px,color:#0b0f14
  classDef thumb fill:#c8ece0,stroke:#0b5946,stroke-width:3px,color:#06241c

  class lock doomed
  class xl1,xl2,xl3,xr1,xr2 dead
  class q,w,e,r,t,a,s,d,f,g,z,x,c,v,b,y,u,i,o,p,h,j,k,l,bspc,n,m,cma,dot,grv core
  class cmd,spc,sft,ctl,ent,alt thumb
```

The Ximi2 has one thing the Corne never had: a four-key cluster per half, beyond the 42, carrying hyper chords, window-management arrows, browser refresh and a duplicate of the record hotkey. This document used to treat that cluster as something to drain and then switch off, and it had a whole stage for the job. **That is withdrawn.** The cluster is work-board hardware on a board that is staying at work; emptying it buys nothing, and eight keys of free capacity are not a debt.

What parity actually requires is narrower than removal, and it is a rule about tenancy rather than about hardware: **nothing may have its only home on those eight keys.** Anything bound there has to be reachable inside the 36-key core as well, so no habit your hands form at work goes missing when you sit down at home. Under that rule the cluster is a bonus — a second, shorter route to things you can already reach — and the duplicate hyper-M and the duplicate record hotkey stop being defects, because duplication is exactly what that hardware is now for. The same reasoning covers the rotary encoders: Ximi2-only, no Toucan2 equivalent, and no reason to strip them as long as nothing depends on them alone.

### Why most of your design already fits

Your punctuation system — the glyph-shape combos, the vertical pairs, the mnemonics you actually internalized — lives entirely inside the thirty keys you keep, on **both** boards. Every one of those combos is single-hand, so narrowing the halves changes nothing about how they feel.

```mermaid
%%{init: {'theme':'base','themeVariables':{'primaryColor':'#eef1f5','primaryTextColor':'#0b0f14','primaryBorderColor':'#5b6673','nodeTextColor':'#0b0f14','textColor':'#0b0f14','mainBkg':'#eef1f5','fontSize':'18px'}}}%%
block
columns 11

  q w e r t space y u i o p
  a s d f g space h j k l bspc["⌫"]
  z x c v b space n m cma[","] dot["."] grv[" `"]

  classDef punc fill:#bfe6f0,stroke:#0a4f63,stroke-width:3px,color:#04222b
  classDef core fill:#eef1f5,stroke:#5b6673,stroke-width:2px,color:#0b0f14

  class e,r,s,d,f,g,t,x,v,b,j,k,l,m,cma,dot punc
  class q,w,a,z,c,y,u,i,o,p,h,bspc,n,grv core
```

`d`+`r` traces a `/` running up and to the right. `f`+`e` traces a `\` running down and to the right. `f`+`s` gives dash, and `x`+`v` — directly below it, same two columns — gives underscore. `t`+`g` for semicolon, `g`+`b` for pipe. All eight of those now exist on both boards on exactly the same letters, which is what made parity nearer than it looked.

Notice what those pairs have in common: not one sits on two adjacent columns of the same hand. `f`+`s` skips `d`. `x`+`v` skips `c`. `j`+`l` skips `k`, and `m`+`.` skips the comma. `t`+`g` and `g`+`b` are vertical, inside a single column — and the qualifier matters: each spans one row, not two, which is what makes them comfortable rather than merely safe. The newest combos obey it too: `i`+`p` skips `o`, `k`+`⌫` skips `l`, `q`+`e` — Escape, added since this was written — skips `w`, and `q`+`b` spans the whole hand. That isn't decoration — a typing roll travels across *neighbouring* columns, so a combo built on a skipped column, or on one column alone, cannot be caught by a roll. Every combo added from here obeys the same constraint, which rules out tempting pairs like `s`+`d` however convenient they look. The two established diagonals, `d`+`r` and `f`+`e`, and the two bracket pairs, `q`+`s` and `a`+`w`, do cross adjacent columns. They are grandfathered because your hands already run them daily, not because they set a precedent — and that sentence now needs a footnote, because one combo has since been added on exactly that precedent.

There is a fifth exception, and it is different in kind from those four: it was not inherited, it was chosen. All sixteen Bluetooth combos hang off one anchor, `o`+`p` — two adjacent columns of the same hand, the exact shape this rule exists to forbid — and `t`-`o`-`p` is worse still, a triple that "top" and "stop" roll straight through inside the combo timeout. The rule was broken knowingly, and paid for in a different currency: every Bluetooth combo carries `require-prior-idle-ms = <250>`, so none of them can fire unless the keyboard was already idle, which mid-word it never is. That number is load-bearing, not decoration — lower it and the adjacency rule reasserts itself as a bug. The honest statement of the trade is that the adjacency rule and `require-prior-idle-ms` defend against the same thing, a roll, and where a whole subsystem wants one memorable posture the timing guard can stand in for the geometry. It is the only place in either keymap where that substitution is taken, it is taken deliberately, and it should stay the only one.

And there is now a sixth, `d`+`v` for `?`, which is the first *diagonal* taken by choice rather than by inheritance, and the weakest-defended combo in either keymap. `d` is the middle column and `v` is the index column, so the pair breaks the rule outright. Three things were understood when it was taken. The first is that it does not invent a kind of exception, it copies one: `d`+`v` is `f`+`e` moved a row down — middle-column home plus index-column bottom, the same down-right diagonal — and `d`+`r`'s exposure to digraphs is comparable, "draw", "drive", "address" and "hundred" against "advance", "advice" and "adventure". That makes the bet empirical and testable against your own hands rather than against a principle: if `d`+`r` has never fired a slash while you typed "address", `d`+`v` will not fire a `?` while you type "advice". Same geometry class, same 150 ms window, one row apart. If `d`+`r` ever does misfire, this combo is the second casualty and the two move together.

The second is that the Bluetooth escape hatch is not available here, and that is a limit on the trick which this document had not stated. `require-prior-idle-ms` works for the radio because the radio is a subsystem you enter deliberately, from a standing stop. `?` is punctuation: you type it mid-word, "what?" lands with the hand already in motion, and an idle guard would block precisely the case you want and nothing else. So the timing guard can substitute for the geometry only where the thing behind the combo is entered rather than typed — which is why the Bluetooth substitution stays the only one of its kind even though a second rule break has now happened.

The third is what the two rejected candidates taught, and they are more instructive than the choice. `a`+`x` was the first instinct: pinky column plus ring column, adjacent, skipping nothing, with `a`→`x` rolls waiting in "tax", "max", "relax" and "galaxy". `s`+`c` was proposed to spare the pinky and is worse — ring plus middle, adjacent again, and "sc" is a far commoner digraph, in "scan", "scope", "score", "discover" and "escape". The lesson is that **the rule tracks column adjacency, not finger strength**, and optimising for the stronger finger walks you straight into a more common roll. `w`+`x` was reconsidered too and dropped on ergonomics rather than geometry: a full-column vertical spans two rows, and you cannot press both ends with one finger, which is exactly what `t`+`g` and `g`+`b` get right by spanning one.

---

## What you learned by doing it

None of this was in the original plan. All of it came out of actually moving keys, and it is the most durable thing the migration has produced so far.

**Tap dances are a tax you stopped paying.** The Ximi2's dance table is down to two live entries out of thirty-two slots — Shift on the left thumb, which becomes Caps Word on a double-tap and Caps Lock on a tap-then-hold, and the Ctrl-and-record dance on the right. Everything else — the Cmd dance, the Alt dance that wrapped a one-shot modifier, the dance on the period key, the dance on backtick, the dance that hung the lock screen off a triple-tap — became a plain modifier or a plain key, and every one of those changes made the board feel faster the same day. A tap dance charges its tapping term on the common case so the rare case can exist, and once you priced that honestly there was almost nothing left worth buying. The Corne is where the bill was largest, and the port paid it: `gui5` and `alt5` are deleted, so the **four-hundred-millisecond** tapping term is off Cmd and Alt — the two modifiers your zellij config leans on hardest — and both are plain keys now. `td10`, which hung the lock screen off a triple-tap, went with them.

The Ctrl dance is the one exception and it survives on purpose, on both boards — `td[1]` on the Ximi2, `control_record` on the Corne, both at 210 ms. Tap Ctrl, hold Ctrl, double-tap for the record hotkey. It was always a bill being paid on the Corne's behalf, and the port kept it deliberately rather than taking the easy win: the Corne still has no spare key for that hotkey, every position that could take it is either load-bearing or already doomed, so double-Ctrl is the shape that habit has to keep. It stays until the Corne has somewhere better to put it.

The left thumb is the one place the purge did not hold, and it took a reversal to get there. The Ximi2's Shift is a dance, `tap_dance[0]`, and it carries both caps behaviours — tap for Shift, double-tap for Caps Word, tap-then-hold for `M11`, which is Caps Lock. ZMK has no tap-then-hold slot at all, so the third behaviour could not be ported as written. What this document said next was that the double-tap substitute was the obvious answer and the wrong one, because it would fire the moment you typed two capitals in a row, and that the Corne's thumb should therefore stay a plain Shift.

The first half of that was a misreading of ZMK's tap-dance. A held Shift followed by a letter resolves **on the letter** — the interrupt path — and not on a second tap, so two capitals in a row never arm the double-tap. Only a deliberate pair of taps does. With that objection gone the Corne now carries `shift_caps` at 200 ms, matching the Ximi2's term: tap for Shift, double-tap for `&caps_word`, and Caps Lock on the **third** tap rather than a hold, which is the closest gesture ZMK can express. It is bound on layers 0 and 1, as on the Ximi2; layers 2, 3 and 5 keep a plain `&kp LSHFT`, because the dance buys nothing on nav, number and F-key layers and would only tax the modifier; and layer 4's left inner thumb stays a one-tap `&caps_word` on both boards, so Caps Word is reachable from base without a layer hop.

The cost argument that this document used to make against it is still the right argument, and it is worth keeping on the page next to the decision that overruled it. A tap-dance on Shift charges a tapping term on the thumb you hold most often, which is exactly the bill the `gui5`/`alt5` purge tore up. What changed the answer was not the price but what was being bought: the thumb was originally proposed as a way to rehouse a key you press a few times a week, which is not worth a term on Shift, and that framing is what this section rejected. The reason it is paid now is open question 13's own rule — **a capability that exists on only one board is a bug even when that board is the one you use most** — and the gesture was missing entirely on the Corne rather than merely relocated. The term is also narrower than the one that was refused: 200 ms rather than 400, on two layers rather than six, with the modifier left plain everywhere the dance earns nothing. If it turns out to cost real typing speed on the base layer, the honest fix is to drop it back to layers 1 only or to a plain Shift and keep the core route, not to argue the price down.

Caps Lock is where the gap used to show, and it has closed twice over. When Caps Lock moved onto the Ximi2's thumb it vacated layer 4's left outer column, and that key was deleted on both boards — which left the Corne with no Caps Lock binding at all. The Ximi2 put the key back, alongside the gesture rather than instead of it, and the Corne bound the same position; and it has now moved again, off the doomed column and onto layer 4's right pinky bottom — the key that carries backtick on the base layer — on both boards. So it is inside the 36-key core at last, on a key that is not going away. And the asymmetry has closed too: both boards now reach Caps Lock by two routes, the core key and the left thumb's dance, with only the gesture's shape differing — tap-then-hold on the Ximi2, triple-tap on the Corne, because ZMK has no hold slot to offer. Same capability, same positions, different final gesture.

**Combo overlaps are acceptable.** Thirteen subset relationships exist among the Ximi2's twenty-five live combos, and the same thirteen exist among the Corne's forty-one — `f`+`s` inside all four home-row layer combos, `f`+`e` inside both nav ones, `j`+`l` inside `j`+`k`+`l`, `m`+`.` inside `m`+`,`+`.`, `x`+`v` inside both layer-5 combos, and the three-key combos inside their own four-key locking variants. The Corne has since added four of its own, all inside the Bluetooth grid: `o`+`p` plus `z`, `x`, `c` or `v` each sits inside the six-key clear-everything. The old reading of that was a design defect. The correct reading is narrower: QMK does not fire a combo the instant its keys are all down. While a longer combo containing those same keys is still reachable, it waits. `x`+`v` does not escape merely because `x` and `v` are both held; it escapes only when the third key lands after `COMBO_TERM` has already expired. That is a timing bug, not an always-on one — it fires on a slow roll and never on a fast one, which makes it the one class of bug that gets rarer the more you use the board. ZMK defers to the longer combo the same way, so the reading transferred to the Corne intact rather than needing to be re-argued there. Not worth retraining deep muscle memory to remove.

**When a punctuation combo collides with a layer combo, move the layer combo, not the punctuation.** Punctuation is older muscle memory and it is reflexive; a layer gesture is deliberate and rare, so the deliberate one is always the cheaper thing to re-teach. This is the principle that briefly turned `x`+`c`+`v` into `x`+`c`+`b`, stepping the layer-5 entrance clear of the underscore. It worked exactly as predicted, and it went back once overlaps were accepted — the premise underneath it had changed, not the reasoning. The principle still governs anything newly added.

**Retire a key by killing it, not by dual-homing it — when failure is cheap.** The original plan said keep both homes live for two to three weeks and let your hands drift to whichever is cheaper. In practice a dead key turns out to be *feedback*. You press it, nothing happens, and you learn precisely which habit hasn't migrated and how often. Dual-homing hides exactly that signal, because the old key keeps paying and your hands never have a reason to stop. Killing the right column outright cost a few days of mild irritation and bought a complete inventory of what still reached outward. Dual-homing looked like the right tool for Tab and Escape, where a miss is expensive mid-flow rather than merely annoying — that is the distinction, not a blanket rule either way. Escape has since spent its exception and been killed anyway, on the reasoning that a combo you are unsure of is tested by having to use it; the fallback is gone, so if `q`+`e` fails under load the answer is a better home for Escape, not the outer key back. Tab was the last candidate for the tool and never used it either: it moved to the thumb in one edit, with the outer key killed in the same pass.

**Swap a redundant key for an orphan.** Backtick took the slash key on the right pinky bottom. Slash surrendered it without complaint, because slash already had a home: `d`+`r` has traced that glyph for as long as you have had combos, and your hands reach for the combo rather than the key. So the pinky position was housing the glyph that needed housing least, while backtick — which you need constantly for code fences — had a key or it had nothing. One move retired two doomed keys instead of one, because tilde is nothing but shifted backtick, so it left the right outer column in the same stroke. Look for that shape again: a base key whose glyph is already reachable another way is not occupied, it is available.

**A pure modifier's tap slot is free real estate — and `mod`+`⇥` is the densest collision on the board.** This is the previous principle one level down. Alt is held and never tapped, so half of that key was doing nothing, and half a key is what Tab needed. But the other half of the lesson cost three rejected designs to learn, and it generalises past Tab. Every thumb modifier has a real `mod`+`⇥` chord behind it — `⌘⇥` is the macOS switcher, `⌃⇥` cycles browser and IDE tabs, `⇧⇥` moves backwards through anything — and a key cannot produce its own hold and its own tap at the same instant, so whichever modifier hosts Tab loses its own `+⇥`. Geometry adds a second casualty: the two keys of a `mod`+`⇥` chord have to sit on opposite halves, so hosting Tab on the right outer thumb also makes `⌃⇥` awkward, Ctrl being the other right thumb. The move was still right, because `⌥⇥` is the one of the four that macOS does not use natively and it is precomposable on a layer. Before hosting any frequently-chorded key on a modifier, list that modifier's own chords first.

**Base-layer positions are the scarcest resource, because they are the only combo-eligible ones.** Every base position you spend on something reachable elsewhere is a combo you cannot define later. That is the real argument against sugar macros on base keys: `./` and `~/` are conveniences, they save two keystrokes each, and neither belongs anywhere near the base layer. Put them on a layer, where the cost is a layer hop and the base position stays available.

**Frequency-weight the good slots.** The symbols layer had to route around one fixed point — `⌥6` could not move — and everything else bent to it. Given that, the high-frequency `⌥←`/`⌥→` pair took the tight adjacent slot on the bottom row where the hand already goes, paying for it with `⌥3`, which is gone and unmissed. The rarely-used `⌥⌃←`/`⌥⌃→` took the wide bookend spread across the top row instead. The awkward position goes to the thing you press least; that is the whole rule, and it is easy to get backwards when you lay out a layer by category instead of by frequency.

---

## Two Vial mechanics that will bite you, and one that limits parity

**These are Vial mechanics, not keyboard mechanics.** Both cost a debugging round on the work board, both produce no error and no log line, and neither one exists on the Corne. The ZMK contrast at the end of this section is the form the same discipline takes there, and it is the form that governed the port.

**Combos match resolved keycodes, not key positions.** Vial stores a combo as a list of keycodes and fires it when the keys you are holding currently *produce* those keycodes. Wrap a base-layer key in `TD()` and it stops producing its own keycode — it produces the dance — so it silently drops out of every combo that named the underlying keycode. The combo stays in the file, looks correct in the editor, and never fires again. Putting `.` behind a tap dance is what killed `m`+`.` → `=`, and since `=` existed nowhere else in the layout, equals stopped existing on the keyboard entirely. The same mechanism had already killed `ESC`+`` ` `` earlier, when backtick went behind a dance of its own. The corollary is the rule worth keeping: on Vial, a combo whose trigger keycode is absent from the base layer is inert.

**Vial's double-tap slot replaces both taps rather than adding a second one.** Set it to the same keycode as the tap action, on the reasoning that two taps should obviously do the thing twice, and you get the opposite: two taps emit one character. Leave the slot at `KC_NO` and two taps give you the tap keycode twice, the way an ordinary key does. This is what broke ```` ``` ```` on the backtick key — the dance was eating backticks in pairs and the fence never closed.

**Vial has a mod-morph, `key_override`, but it fires too late to fix a mod-tap.** This is the third mechanic, and unlike the two above it is a capability limit rather than a bug. `akiva.vil` carries a `key_override` table of thirty-two slots, none used, with QMK's exact fields — `trigger`, `replacement`, `layers` as a bitmask, `trigger_mods`, `negative_mod_mask`, `suppressed_mods`, `options`. `suppressed_mods` is the inverse of ZMK's `keep-mods`, `negative_mod_mask` has no ZMK counterpart, and the whole table is runtime-editable with no recompile, which puts Vial ahead of ZMK here. What it cannot do is pre-empt a mod-tap, and for exactly the reason mechanic 1 gives: overrides act on **resolved** keycodes. `LALT_T(KC_TAB)` does not produce `KC_TAB` until release, so an override on `KC_TAB` fires at the same late moment and inherits the same exposure. Whether `trigger` can name the `LALT_T(KC_TAB)` keycode itself — QMK runs `process_key_override` before tap-hold resolution, so in principle it might — is undocumented and is open question 14. There is also no Vial equivalent of `hold-trigger-key-positions`: Chordal Hold is opposite-hands-only, which here would resolve `⌥h`/`⌥j`/`⌥k`/`⌥l` as taps and break them, and Achordion needs a firmware build. And Vial's tapping term is global, so the Corne's per-behaviour 180 ms cannot be mirrored without retiming every `LT()`.

**A ZMK hold-tap needs its own `bindings`, and the parameters at the call site are not a substitute.** This one is cheap to fix and costs a build to find. `alt_tab` was written with `flavor`, `tapping-term-ms` and `hold-trigger-key-positions` and no `bindings` property, on the assumption that `&alt_tab LALT TAB` at the call site said everything — it does not. A hold-tap's `bindings` names the two *behaviours* the hold and tap parameters are applied to, and the call site supplies only the parameters, so `bindings = <&kp>, <&kp>;` is what turns `LALT` into `&kp LALT` and `TAB` into `&kp TAB`. Omit it and devicetree refuses the node outright: `'bindings' is marked as required in 'properties:' in zmk,behavior-hold-tap.yaml, but does not appear in <Node /behaviors/alt_tab>`. Unlike the two Vial traps above this one fails loudly at build time rather than silently on the board, which is the whole difference between a compiled keymap and a runtime-edited one.

**Neither of the first two traps exists in ZMK, and the mirror image does.** A ZMK combo is declared as `key-positions = <14 16>` — it binds to *positions*, not to keycodes. Wrapping position 16 in a tap-dance, a hold-tap or anything else does not orphan a single combo, because the combo never asked what that key produces. The `TD()` hazard simply is not there. ZMK tap-dances are likewise a plain list of bindings rather than Vial's four fixed slots, so the double-tap substitution behaviour does not apply either. What ZMK breaks on instead is the opposite move: shift a key to a different *position* and every combo naming the old position now fires on whatever moved into it, silently and wrongly. Vial's combos survive a key move and break when a key is wrapped; ZMK's survive a wrap and break when a key moves. Different failure modes, same discipline — re-read the combo table after every change, and know which question to ask of which board.

---

# The journey

## Stage one — retire the right column

**Status — done on both boards, bar one tenant.**

This is the stage that proved the whole thing is possible, and it went faster than the plan allowed for. All three keys of the right outer column are bound to nothing on every layer, with exactly one exception: `M10`, lock screen, still sits on the base layer as that column's sole remaining tenant. It is deferred rather than forgotten, and it needs a core home. The Corne now reads identically, tenant included — the port copied the deferral rather than papering over it, which is the right call while the answer is still unknown, and the wrong one to leave standing for long.

Four things about how it actually happened diverge from what was written down, and in three of the four cases the real version is better.

**Backtick went to the slash key, not to the symbols layer.** The plan proposed parking it on layer 1, which has room. What happened instead retires two doomed keys with one edit and charges no layer hop for a code fence. This is the *swap a redundant key for an orphan* principle, and it was discovered here.

**Browser back and forward became core combos, not nav-layer keys.** The plan put them on the nav layer directly under the left and right arrows, in matching columns, which is tidy and which charges a layer hop for something you fire dozens of times an hour while reading. What they got is `i`+`p` for forward and `k`+`⌫` for back — two-key rolls on the base layer, both skipping a column, both on the right hand where those actions have always lived. The reach that used to go outward now rolls inward and nothing changes sides.

**Layer access was rebuilt wholesale, not patched.** The plan asked only that the sticky layer-5 entrance be deleted and a single deliberate switch kept. What exists now is a uniform pair applied to every layer at once:

| Layer | Momentary | Locked |
|---|---|---|
| 2 — nav | `s`+`e`+`f` | `a`+`s`+`e`+`f` |
| 3 — numpad | `s`+`d`+`f` | `a`+`s`+`d`+`f` |
| 4 — function | `j`+`k`+`l` and `m`+`,`+`.`, both one-shot | none, by choice |
| 5 — F-keys | `x`+`c`+`v` | `z`+`x`+`c`+`v` |

Read the shapes and the scheme teaches itself: the momentary gesture is three keys, the locked one is the same three with the pinky added ahead of them. You never have to remember which layer works which way. Layer 4 is the deliberate exception with no locked form — the function layer is the one you enter to press exactly one key, so one-shot is the whole point and a lock would be a trap. Layer 2 is additionally a hold on the left middle thumb, because it is the layer you live in. The return trip is final too: `TO(0)` sits on the inner-index top key, the `t` position, on layers 2, 3, 4 and 5, and that column survives the left-column retirement, so the escape hatch you are learning now is the escape hatch you keep.

**The dash collision was left in place.** The plan's fix was to move the punctuation — dash to `d`+`g`, underscore to `c`+`b`. That was not taken. The mirror-image fix was taken instead and then, once overlaps were accepted as a timing cost rather than a defect, even that was reverted. `f`+`s` still sits inside all four home-row layer combos and will keep doing so.

Two costs were accepted with eyes open, and they are decisions rather than bugs. The first is a **direction reversal**: the symbols layer's bottom-row pinky used to be `⌥⌃←` and is now `⌥→`, pointing the other way. It is the only genuine muscle-memory inversion in the whole redesign, and it is the price of frequency-weighting the good slots. The second is the **combo overlaps**, thirteen of them, for the reason given above.

*You finished this stage when a week went by without the right column being missed — which it did, almost immediately, and more easily than anyone expected.*

## Stage two — bring the Corne to parity

**Status — done, in one sitting, which is how it had to be done.**

The Corne was left behind on purpose and the deferral paid for itself. A change on the work board is tested against eight hours a day of real typing inside a week, where the same change at home would take a month to earn the same confidence, and porting a design nobody has lived in is how you end up porting it twice. That was a tactical choice, and the reason it could not be allowed to become a structural one is worth keeping on the page even now that it is settled.

It is this. **Every evening on an unported Corne was practice on the old layout.** Your hands spent those hours rehearsing exactly the reaches you are trying to retire — the outer column, the old bracket combos, the four-hundred-millisecond modifiers — and the two habits competed the entire time. That is the same mechanism that made the work board the right place to learn, running in reverse and undoing the learning. Every week the gap stayed open leaked away some of stage one's gain between six in the evening and nine in the morning. That is why this stage jumped the queue, and it is the argument to reach for the next time a board is tempted to run ahead alone.

The work was mostly mechanical, because the two boards were never far apart. The punctuation combos were already on identical letters. What changed:

| What | On the Corne before | Now |
|---|---|---|
| Right outer column | browser forward, browser back, tilde, on every layer | `&none` everywhere except `m10`, the lock screen — the Ximi2's deferral, copied |
| Backtick | left outer bottom | right pinky bottom, taking the slash key |
| Slash | right pinky bottom | combo only, `d`+`r`, which already existed |
| `[` and `]` | Tab+`a` and `q`+Escape — both anchored on doomed keys | `q`+`s` and `a`+`w` |
| Browser back / forward | outer-column keys | `k`+`⌫` and `i`+`p` |
| Layer access | `s`+`d`+`f`, `s`+`e`+`f`, `.`+`l`, sticky variants, 60-second release | the uniform momentary/locked table above, and the 60-second window gone |
| Cmd, Alt | tap-dances at 400 ms | plain modifiers; `gui5`, `alt5` and `td10` deleted |
| Ctrl | `control_record` tap-dance | kept, at the Ximi2's 210 ms — the one dance that earns its term |
| Mouse | movement, scroll, `CONFIG_ZMK_MOUSE`, both mouse headers | deleted; the four pointer combos kept, and the deprecated flag replaced by `CONFIG_ZMK_POINTING` |
| Bluetooth | a select/disconnect/clear grid of keys on layer 5 | sixteen combos on the `o`+`p` anchor — no keymap position on either board |
| Combo scope | none — all 32 fire on all six layers | 41 combos, every one of them `layers = <0>` |
| Layer-tap protection | none of any kind | `quick-tap-ms = 200`, `require-prior-idle-ms = 125`, `flavor = "tap-preferred"` |
| Macros | twelve legacy named macros, nine of them duplicates | renumbered to the Vial table, `m0`..`m27` with the gaps the Vial table has, plus `btclr0`-`btclr4`, which the Ximi2 has no use for — twenty-five in all |

The last row was stage three's work, pulled forward because you cannot port position-for-position against a file you do not believe. The ASCII diagrams were regenerated in the same pass, for the same reason.

Three differences still do **not** port, and the list is shorter than it reads, because one of the three used to be Bluetooth and is not any more. The radio was always the awkward case: the Corne needs select, disconnect and clear, and the Ximi2, being wired, has no use for any of it. The first landing was clean enough — the Ximi2 leaves its whole left half transparent on layer 5, so the grid, soft off, Studio unlock and the bootloader moved in without displacing a binding that exists on the other board. Clean, and still fifteen positions on which the two boards said different things.

Bluetooth has now left the keymap altogether. It is sixteen combos hanging off one shared anchor, `o`+`p`, with a third key naming the profile and that third key's row picking the verb: **top row selects, home row disconnects, bottom row clears**, plus `o`+`p`+`z`+`x`+`c`+`v` to clear everything. Every key in the grid is inside the 36-key core, so all sixteen survive the move to a smaller board — which the old four bottom-row pairs also did, but they only reached profiles 0 to 2 and a clear-current, and they were four unrelated shapes rather than one posture. Two ZMK facts shaped the rest of it. **Profiles count from 0**, so the first key of each row is profile 0, not profile 1, and there is no profile 5. And `BT_CLR` takes no profile index at all — it clears whichever profile is current — so clearing a *named* profile is a sequence rather than a keypress: select, wait 30 ms, clear. That is why the macro table grew five entries it did not have, `btclr0`-`btclr4`. Two budgets in `corne.conf` had to be raised to hold the grid: `CONFIG_ZMK_COMBO_MAX_COMBOS_PER_KEY=24`, because `p` now sits in seventeen combos against a default of five, and `CONFIG_ZMK_COMBO_MAX_KEYS_PER_COMBO=6`, because clear-everything needs six keys against a default of four.

So the **first** difference that remains is not Bluetooth but what is left standing beside where it used to be: five keys on layer 5 — `&studio_unlock` on `z`, `&soft_off` on `g`, `&bootloader` on `b`, and `&out OUT_USB` on `q` with `&out OUT_BLE` on `a` — positioned so the layer-entry combo `x`+`c`+`v` touches none of them. All five stay because ZMK has them and a wired QMK board has no equivalent to port them to — they are a capability difference, not a leftover. The two endpoint keys are the newest of them and they exist because ZMK Studio asks for them: with the board paired over Bluetooth and Studio attached over USB, output has to be pointed at the endpoint Studio was opened on. Everything else on that half is `&trans` again, and the layer is renamed from `bt` to `fkeys` because that is now all it is. **Layer 5 went from fifteen divergent positions to three,** which is the largest parity gain since the port itself, and it came from deleting keys rather than finding homes for them. The endpoint pair has since taken it back to five. The **second** difference is the `&lt` layer-tap protection, which has no QMK equivalent knob and so stays a Corne-only improvement. There **is** a third again, and stage four created it: the `alt_or_tab` mod-morph that makes Shift+Tab a keydown event. Vial's `key_override` is the right shape for it but acts on resolved keycodes, so it cannot reach a mod-tap — see open question 14 — and until that is tested the Ximi2's Shift+Tab depends on releasing Shift last. Caps Lock used to be the third and no longer is: the Ximi2 restored the layer 4 key beside its thumb gesture, the Corne bound the same position, and the Corne has since taken the thumb gesture too, as a triple-tap.

Two things about that outcome are worth being explicit about rather than letting the parity number carry them. It broke this document's own combo-adjacency rule, knowingly, and bought its way out with `require-prior-idle-ms = <250>` — the argument is above, under the rule it broke. And Bluetooth now has **no fallback key anywhere**. If a combo stops firing — a debounce change, a timeout change, a dead switch under `o` or `p` — there is no position to press instead, and re-pairing the board needs the physical reset button. That is the right trade while `o` and `p` are healthy, and it is the reason no Bluetooth combo should be moved or retimed casually.

And the two Vial mechanics above are Vial's, not the keyboard's — ZMK combos bind to key positions, so a behaviour wrapped around a key orphans nothing, and ZMK tap-dances have no double-tap substitution slot. No phantom constraint was ported. The *discipline* was, inverted: in ZMK it is moving a key to a new position that breaks the combos naming the old one, which is why every combo in the file carries the letters it means in a comment beside the position numbers.

It was done as one sitting, not as a drip. A half-ported Corne would have been worse than an unported one, because then neither board is a reliable model of the other.

*You've finished this stage when you can sit down at either keyboard and not notice which one it is.* Which is now true everywhere except the outer columns — and those are dead, doomed or down to one tenant on both.

## Stage three — housekeeping, both boards

**Status — done on both boards. Nothing on the list is still open.**

None of this changes anything you press. All of it blocks clear thinking, and the thinking left to do is about the left outer column.

On the **Ximi2**, in rough order of how much confusion it causes. Four of the seven items below have quietly closed since they were written, which is worth recording rather than deleting — it says something about which kinds of clutter actually get cleaned:

- ~~**Ten unused tap-dance slots.**~~ Closed. The dance table is compacted: `td[0]` is Shift with double-tap Caps Word and tap-then-hold Caps Lock, `td[1]` is the Ctrl-and-record dance, and every other slot is genuinely empty rather than half-populated. The old `TD(2)`/`TD(4)` numbering in your notes is stale.
- ~~**Orphan macros `M11`, `M21`, `M25`, `M26`.**~~ Closed, and `M11` is not merely non-orphaned — it is Caps Lock again, bound on layer 4's right pinky bottom key on both boards, inside the core. The duplication that is left is `M28` and `M29`, byte-for-byte copies of `M18` and `M19` — the `⌘⌥` up and down arrows. Both pairs live on the extra cluster, which is staying, so that duplication is now a feature of a bonus cluster rather than a defect awaiting a stage.
- ~~**`M4` and `M12` are both `./`.**~~ Closed. `M12` is `../`, and the function layer's bottom row now runs `~/`, `./`, `../` inward. The shuffle cost `M22`, `thisisunsafe`, its key: it is defined and unbound on both boards, which is an acceptable price for three distinct path prefixes on three keys.
- ~~**Combo slot `UI31` is malformed.**~~ Closed. The combo table was compacted in the same pass that added `q`+`e` for Escape, and the stranded `KC_BTN1` went with the empty slots.
- ~~**Layer 3's duplicated `9`.**~~ Closed. The right pinky is bound to nothing now, which is honest: it was never a second nine, it was a dead key wearing a label.
- ~~**Layer 2 binds `⇧⌘E` on two different keys.**~~ Closed, and closed generously: the whole bottom-left row went with it, `⌘⌥T`, F4 and Shift+F4 included. That is **five free positions inside the core on the nav layer** — the largest block of empty, combo-adjacent, both-boards space the migration has, and the obvious place to look first when the left outer column needs somewhere to go.
- ~~**`td[2]` gates layer 1's Shift** behind a 200 ms dance while base-layer Shift is plain.~~ Closed, though not the way this bullet wanted: layer 1 and the base layer now carry the *same* dance, `td[0]`, so the inconsistency is gone but the tax is on both. That dance is the one binding the Corne could not mirror.

On the **Corne**, what used to be a bigger and older pile is cleared. Twelve unreferenced macros are gone, nine of them exact duplicates, including the two copies each of fold, unfold and fold-all, and including the pair whose names were inverted — where `fold` bound unfold and `expand` bound fold, which is the single most misleading thing that has ever been in either file. The ASCII diagrams were regenerated against the bindings rather than patched. `scroll-right`, bound on two adjacent keys while `scroll-left` appeared nowhere, went out with mouse emulation. The duplicate `thisisunsafe` is gone, every layer has a `display-name`, and the macros are numbered to the Vial table so the two files can be read side by side. Most of that died with stage two, exactly as predicted; nothing survived it needing a second pass.

Three of the Ximi2's defects were reproduced on the Corne rather than fixed — layer 3's duplicate `9`, layer 2's two `⇧⌘E` keys, and the two macros that both type `./` — and that reproduction is worth keeping on the page now that all three are gone, because it is the thing that made fixing them cheap. They were copied on the rule that two identical boards are worth more than one correct key, and a file claiming to be a position-for-position port loses the property that makes it useful the moment it starts quietly improving on its source. The prediction attached to that rule was: *fix them on the Ximi2 and the Corne follows in the same commit.* That is precisely what happened — one Vial pass, one keymap pass, no divergence at any point. The discipline paid out exactly as advertised, which is the best argument for keeping it the next time a defect looks trivially fixable on one board.

*You've finished this stage when you can read either board's layer map and believe it, and nothing in either file is defined twice.* Both are now true, with no exceptions outstanding.

One more item closed here rather than executed: **this document used to have a fourth stage, to drain and switch off the Ximi2's extra cluster.** It is deleted, not deferred. The cluster is staying, for the reasons under *What is still on the block* above, and what replaced the stage is a one-line rule — nothing may have its only home out there. Every stage after it moved up a number.

## Stage four — find homes for the left outer column

**Status — every tenant is executed or rehoused, on both boards. The last of them, Tab, is on the right outer thumb's tap slot. This is where the open questions live, and two of the ones that mattered most are now struck through.**

This is the stage the document cannot finish for you, and it has very nearly finished itself. The right column was easy because almost nothing on it was load-bearing. The left column was the opposite: it held two of the keys you press most, and none of the decisions had been made. All of them have been made since, and every one of them landed on both boards at once — which was the whole point of doing stage two first, and is the dividend it pays.

Here is everything currently living on it, across all six layers, on both boards:

| Layer | Top | Middle | Bottom |
|---|---|---|---|
| 0 — base | dead — Tab is the right outer thumb's tap now | dead — Escape is `q`+`e` now | dead — backtick left in stage two |
| 1 — symbols | dead — layer 1 is hold-only and cannot strand you | dead | dead — `?` unbound in this stage |
| 2 — nav | `TO(0)` | dead | dead — `LCTL(GRAVE)` dropped in this stage |
| 3 — numpad | `TO(0)` | dead — the copy here is deleted | dead |
| 4 — function | `TO(0)` | dead — the copy here is deleted | dead — Caps Lock moved into the core |
| 5 — F-keys | `TO(0)` | dead | dead |

Read that table and what is left of the column is one redundant binding repeated four times. The middle and bottom positions are `&none` on all six layers on both boards, and the top position is now `&none` on the base layer and on layer 1. `TO(0)` is already prepared on the inner-index top key — the `t` position — on layers 2, 3, 4 and 5, and that column survives, which is what let the middle key's copies on layers 3 and 4 be deleted outright rather than relocated. Layer 1 used to be the exception, on the grounds that its `t` position is `$` and position 0 was the only way home from the symbols layer. That premise was wrong: layer 1 has no locking entry at all, so you are never on it without holding the thumb, and releasing the thumb *is* the way home. Its copy is deleted. What survives is the copy on layers 2 to 5, which duplicates position 5 on each of them, and which nothing prevents deleting.

The port took a position on half of that without being asked to. Every `DF(0)` became `&to 0` on the Corne, because `&to 0` is what the rest of the map already used and nothing in the file could say what `DF(0)` was buying. That is almost certainly right — the default layer is 0 and nothing ever sets it elsewhere — but it is a decision taken in a porting pass rather than a tested equivalence, and it is recorded here as such. `DF(0)` and `TO(0)` are still not the same instruction, and layer 1 still has no `t`-position escape hatch at all.

**Escape** was the hard one, and it is now executed. The candidate here was `w`+`x` — top and bottom of the middle-finger column, skipping the home row. What got tried instead, and what is live on both boards, is **`q`+`e`**: top of the pinky column plus top of the middle column, skipping `w`. It obeys the same constraint by a different route — a skipped column rather than a single one — and it costs nothing that `w`+`x` was going to cost. What has changed since is the other half of that plan. The key was to stay live beside the combo, as the one dual-homing exception the document allowed itself; it is gone instead, on every layer, on both boards, because a combo you are not yet sure of is not tested while the old key is still there to catch you. So the exception was argued for, granted, and then spent unused. Between vim and LLM shells the load arrives within the hour, and if `q`+`e` cannot take it the answer is a better home for Escape — `w`+`x` is still unrefuted — and not the outer key back. Reverting would only teach your hands that the old position still pays.

**Tab** is answered, and the answer is a modifier's tap slot. It sits on the right outer thumb: hold for Alt, tap for Tab. `&alt_or_tab` on the Corne, `LALT_T(KC_TAB)` on the Ximi2, base layer only — layers 1 to 5 keep a plain Alt there.

The principle is worth naming, because it is a level below *swap a redundant key for an orphan*. **A pure modifier's tap slot is free real estate.** Alt is held and never tapped, so half of that key was doing nothing. Not a redundant key — a redundant half of a key.

Three things had to be true for it to be affordable, and each answers an objection this document raises elsewhere.

It is not the `gui5`/`alt5` tax again. A tap-dance charges its tapping term on every press; a hold-tap with `flavor = "hold-preferred"` resolves the hold the instant another key goes down, so `⌥h`/`⌥j`/`⌥k`/`⌥l` cost nothing. The 180 ms runs only when no key follows, which is the keyless hold: `⌥`-click, `⌥`-drag, `⌥`-scroll.

The thumbs are excluded from `hold-trigger-key-positions`. Without that, pressing Shift a few milliseconds *after* the Tab key satisfies hold-preferred's "another key went down" test, and Shift+Tab silently becomes Alt+Shift.

Shift+Tab needed a mod-morph, and this is the part with no Vial equivalent. A hold-tap emits its tap on **release**, so with Shift on the left thumb and Tab on the right, a left thumb that lifts a few milliseconds early lands a plain Tab — which in a Claude Code session cycles the mode the wrong way. No timing knob fixes a late tap. `zmk,behavior-mod-morph` does: with Shift already held the key is a plain `&kp TAB` firing on keydown, and `keep-mods` passes the Shift through. The mask is Shift alone, deliberately — widening it to Cmd and Ctrl would mean this key can never produce Alt while either is held, killing live `⌘⌥x` and `⌃⌥x` chords, and `⌘⇥` needs no help because you hold `⌘` far past the Tab while browsing a switcher.

What the move costs is `⌥⇥` from that key, since Alt is its own hold. That went to layer 1's right pinky home key as a precomposed `⌥⇥`, joining the `⌥`-digit window-management block — one-shot, because a precomposed keycode releases Alt with the Tab and so cannot hold a switcher open and cycle. It displaced `^`, which took the home-row slot and pushed `$` out onto the `t` position, which held a second copy of `$`; `$` is the commoner glyph, but `^` is the one the owner wants on the better finger, and a stage-three "nothing defined twice" violation closed in the same edit. `⌃⇥` is the other casualty and it is geometric rather than logical: Ctrl and Tab are now both right-thumb positions, so the chord wants one thumb on two keys. Layer 2's dead bottom-left row has six free core positions if that turns out to matter.

One worry is **not** discharged, and it arrived with a change made in the same pass. Shift on the left inner thumb is no longer a plain `&kp LSHFT`; it is the `shift_caps` tap-dance. A mod-morph reads the held modifiers at the instant of its own keydown, and a tap-dance emits its binding on the interrupt path rather than when the thumb goes down, so whether that Shift is in the report by the time `alt_or_tab` is evaluated decides which branch Shift+Tab takes — the plain `&kp TAB` this whole design is for, or the hold-tap with the release-order exposure it exists to remove. It is open question 15 and it wants a real keyboard, not a reading of the source.

One worry that *was* already discharged before the move: the `⇥`+`b` prose macro, which would have gone dark the moment Tab did, was re-anchored to `q`+`b` on both boards. Nothing else in either combo table names position 0, or any thumb, so the move broke no combo on either board.

**`?` on layer 1** is decided, and not the way this document proposed. The proposal was that it needed no home at all: slash exists as `d`+`r`, `?` is shifted slash, so Shift plus the combo ought to produce it — a five-minute test. That test was never run and no longer decides anything, because the glyph got a combo of its own instead, `d`+`v`, and layer 1's left outer bottom key is unbound on both boards. Picking the pair took three goes and cost the document a rule. `a`+`x` was the first instinct and `s`+`c` the second, and both are adjacent columns with common digraphs behind them; `d`+`v` is adjacent columns too, and was taken anyway because it is `f`+`e`'s diagonal one row down and those diagonals have never misfired. The full argument is under the adjacency rule above, where it belongs, along with what the two rejections taught. What matters here is that the key is gone, the glyph is reachable, and the combo that reaches it is the one place in the punctuation set standing on nothing but precedent.

**Caps Lock** is resolved, and it took two goes. The prediction was: moved on one board, dropped on the other. What happened is that the Ximi2 took it onto the left thumb's tap-then-hold *and then put the layer 4 key back beside it*, so the gesture is a shortcut rather than the only route — and the Corne, which could not express that gesture as written, bound the key. That left it a tenant of the left outer column again, on layer 4, which is where this section left it. It has since moved **into the core**: layer 4's right pinky bottom, the position that carries backtick on base, on both boards. The reach is the same hand and the same finger as the glyph it sits under, the key survives every column retirement in the plan, and the thumb route the owner proposed for the Corne has since been built after all, as a triple-tap rather than a tap-then-hold — the reversal is above, under the tap-dance lesson. Both boards also keep `caps_word` on layer 4's left inner thumb. So the bottom position of the left outer column is now dead on all six layers, with nothing left to rehouse.

**`LCTL(GRAVE)`** — Ctrl plus backtick, the terminal-cycling chord — is decided, and the decision is to drop it rather than to move it. It is not needed, and the worst case is pressing `⌃` and `` ` `` explicitly, which is two keys that both still exist. Nothing substitutes for it because nothing has to. Unlike most of what happens in this stage, this one closes a divergence instead of opening one: the Ximi2 had already cleared that position and the Corne was the odd board out, so the cheapest fix was also the correct one, and layer 2's left outer bottom key is now dead on both boards.

Add those up and the stage is spent. Seven tenants are settled: `LCTL(GRAVE)` dropped outright, `?` moved to `d`+`v`, Escape replaced by `q`+`e` rather than joined by it, Caps Lock moved into the core, the middle key's two return-to-base copies deleted as redundant, **Tab** moved onto the right outer thumb's tap slot, and layer 1's return-to-base copy deleted once it turned out layer 1 cannot strand you. All three positions are dead on the base layer and on layer 1, on both boards. What is left anywhere on the column is the return-to-base copy on layers 2 to 5, which duplicates position 5 on each of those layers and is one edit per board from gone. The closing line below is true of every tenant that ever needed a home.

*You've finished this stage when every tenant of the left outer column has either a decided new home or a decided execution, on both boards.*

## Stage five — retire the left column

**Status — all three positions are live-fire on the base layer and on layer 1, on both boards, well ahead of this stage's own schedule. What still binds anything is a redundant return-to-base copy on layers 2 to 5. This is the gate.**

The real test. Bind all three positions to nothing on every layer, on both boards at once, and live there for two full weeks. Stage four did nearly all of it without meaning to: the middle and bottom positions are `&none` on all six layers of both boards, and the top position is `&none` on the base layer and on layer 1 now that Tab is on the thumb. So the two weeks are running on the whole column already, and the feedback this stage exists to harvest is arriving whether or not the stage has been declared open. What still binds anything is the return-to-base copy on layers 2 to 5, which duplicates position 5 on each of them — redundant rather than load-bearing, and deletable whenever you want the column formally dead. The instruction to delete the last combos that reference the column had already been carried out ahead of time: `⇥`+`b` was the only one, and it is re-anchored on `q`+`b`. Nothing else in either combo table touches those three positions, or any thumb, so the move cost no combo and the rest can be switched off in a single edit per board with no collateral. The picture below draws the right outer column dead too, because that is where it ends up — which assumes the lock screen has found a core home by then. It has not. Open question 8 is the whole distance between this diagram and the literal truth.

```mermaid
%%{init: {'theme':'base','themeVariables':{'primaryColor':'#eef1f5','primaryTextColor':'#0b0f14','primaryBorderColor':'#5b6673','nodeTextColor':'#0b0f14','textColor':'#0b0f14','mainBkg':'#eef1f5','fontSize':'18px'}}}%%
block
columns 13

  xl1["❌"] q w e r t space y u i o p xr1["❌"]
  xl2["❌"] a s d f g space h j k l bspc["⌫"] xr2["❌"]
  xl3["❌"] z x c v b space n m cma[","] dot["."] grv[" `"] xr3["❌"]
  space:3 cmd["⌘"] spc["␣"] sft["⇧"] space ctl["⌃"] ent["⏎"] alt["⌥<br/>⇥"] space:3

  classDef dead fill:#f2f3f5,stroke:#98a2ae,stroke-width:2px,stroke-dasharray:5 4,color:#5b6673
  classDef core fill:#eef1f5,stroke:#5b6673,stroke-width:2px,color:#0b0f14
  classDef thumb fill:#c8ece0,stroke:#0b5946,stroke-width:3px,color:#06241c

  class xl1,xl2,xl3,xr1,xr2,xr3 dead
  class q,w,e,r,t,a,s,d,f,g,z,x,c,v,b,y,u,i,o,p,h,j,k,l,bspc,n,m,cma,dot,grv core
  class cmd,spc,sft,ctl,ent,alt thumb
```

There is a second way to do this, and it is worth naming because it is free and available today: **physically pull the outer keycaps, or the switches.** On both boards. It costs nothing, it takes minutes, and it is a far stronger commitment device than `&none` — you cannot absent-mindedly press a key that is not there, and there is no config change to quietly revert on a bad afternoon.

The trade-off is real and it cuts the other way. A key that is dead but still physically present is *feedback*. You hit it, nothing happens, and you learn exactly which habit hasn't migrated and how often. That is the whole diagnostic value of these two weeks, and it disappears the moment the keycap is in a drawer — a finger landing on bare plate tells you something happened, but not nearly as precisely. There is also a middle route: run the two weeks with the keys dead but present, harvest the feedback, and pull the caps afterwards as the thing that makes it permanent.

Either way, if a specific key keeps catching you, don't revert the column. Go back to stage four, give that one key a better home, and continue. Reverting teaches your hands that the old position still pays.

*You've finished this stage when two weeks pass, on both boards, and you stop noticing the dead columns.* This is the gate for buying hardware — the only one, and it may now gate two purchases rather than one.

## Stage six — buy the hardware

**Status — not started. Gated on stage five.**

Your layout already runs on it, on both boards. The dead keys simply stop existing physically. There is no migration event, no adjustment day, no flashing anxiety — you will have been typing this layout for weeks.

For home: order the Toucan2's 36-key build, port the keymap to the new shield with the six dead entries dropped per layer, and set up the trackpad. For work there are two routes and they cost very differently. A 36-key Ximi, if such a build exists, is the clean answer. De-keying the board you already own is the free one — pull the outer switches and keep everything else, the extra cluster included and live. That part is already decided: the cluster stays usable, under the rule that nothing has its only home there. Which of the two routes to take is not decided, and both are downstream of the gate.

Two things will surprise you on the Toucan2, both new capability rather than migration. The columnar stagger is steeper than the Corne's and the pitch is tighter — a finger-position adjustment, not a layout one, and days rather than weeks. And the trackpad will quietly change what the nav layer is for. Once pointing and scrolling are gestures, that layer's real job is arrows, word-motion and the browser back/forward you kept. Expect to redesign it, but after a week of living with the trackpad rather than in advance.

---

## Side quest — home row mods

**Status — not started on either board, and genuinely optional.**

All six thumb keys survive on the Toucan2, so the all-thumb modifier strategy keeps working indefinitely. But home row mods are the difference between a 36-key board that feels cramped and one that feels roomy: thumbs stop being a bottleneck, chords stop needing a thumb-and-finger stretch, and thumb capacity frees up for the layer access a smaller board leans on harder.

There is a tension here that has to be named rather than skipped. **Home row mods are hold-taps, and hold-taps are the same family of latency the tap-dance purge was about.** Every one of the eight keys below would gain a timing decision it does not have today, on the eight positions your hands rest on. Someone who just finished stripping the tap dances off both boards because they made typing feel slow is entitled to be suspicious of this, and that suspicion is correct in the general case.

```mermaid
%%{init: {'theme':'base','themeVariables':{'primaryColor':'#eef1f5','primaryTextColor':'#0b0f14','primaryBorderColor':'#5b6673','nodeTextColor':'#0b0f14','textColor':'#0b0f14','mainBkg':'#eef1f5','fontSize':'18px'}}}%%
block
columns 11

  q w e r t space y u i o p
  a["a<br/>⌘"] s["s<br/>⌥"] d["d<br/>⌃"] f["f<br/>⇧"] g space h j["j<br/>⇧"] k["k<br/>⌃"] l["l<br/>⌥"] bspc["⌫<br/>⌘"]
  z x c v b space n m cma[","] dot["."] grv[" `"]
  space:2 cmd["⌘"] spc["␣"] sft["⇧"] space ctl["⌃"] ent["⏎"] alt["⌥<br/>⇥"] space:2

  classDef hrm fill:#e3d2f5,stroke:#4a1f72,stroke-width:3px,color:#240c39
  classDef thumb fill:#c8ece0,stroke:#0b5946,stroke-width:3px,color:#06241c
  classDef core fill:#eef1f5,stroke:#5b6673,stroke-width:2px,color:#0b0f14

  class a,s,d,f,j,k,l,bspc hrm
  class cmd,spc,sft,ctl,ent,alt thumb
  class q,w,e,r,t,g,z,x,c,v,b,y,u,i,o,p,h,n,m,cma,dot,grv core
```

The difference from a tap dance is that a hold-tap's latency can be made conditional, and that is the whole question. **Opposite-hand-only holds** — `hold-trigger-key-positions` on the Corne, Chordal Hold or Achordion on the Ximi2 — resolve a same-hand press as a tap immediately, with no waiting at all, so the tax applies only to cross-hand chords where you were going to hold anyway. Without that setting, home row mods misfire constantly on same-hand rolls and you will give up within a day, correctly. With it, the decision is genuinely different in kind from a tap dance. That one setting decides whether this is tolerable, so test it before committing to anything else here. One caution on the work board: Chordal Hold and Flow Tap are recent QMK features and Vial firmware often lags mainline, so check what your fork supports before planning around it.

Your zellij habits make it more load-bearing than usual. `Alt h/j/k/l` becomes left-hand Alt plus a right-hand letter — a clean opposite-hand chord, and genuinely nicer than today. But `Alt b`, `Alt c`, `Alt v`, `Alt f` and `Alt z` all target left-hand letters, so they must use the *right* hand's Alt. That is exactly what opposite-hand-only holds are for, and exactly what breaks without them.

Keep the thumb mods live throughout. On a day when the home row fights you, your hands fall back to what they know and you lose nothing. Then tune: start around 200 ms and walk down in 10 ms steps until chords feel immediate without false holds mid-word. Expect one to two weeks of misfires before it goes quiet.

This is the stage most people abandon. Abandoning it costs you nothing here — that is why it lives outside the sequence.

---

## Open questions

Unresolved, and listed rather than papered over. Most of stage four is in here.

1. ~~**Does `q`+`e` hold up as Escape?**~~ Answered by execution rather than by test. The key was to stay live beside the combo — the one dual-homing exception the plan allowed — and it is gone instead, on every layer of both boards, because the dual home was hiding the very signal the test needed. So the field trial *is* the test, and it started the moment the key died. The fallback is spent: if `q`+`e` fails under a long vim session or a bad afternoon in an LLM shell, the answer is a different home for Escape, `w`+`x` included, and not the outer key back. Reverting would teach your hands that the old position still pays, which is the one thing this document says never to do.
2. ~~**Where does Tab go?**~~ Answered: the right outer thumb's tap slot. `&alt_or_tab` on the Corne, `LALT_T(KC_TAB)` on the Ximi2 — hold for Alt, tap for Tab, base layer only. The constraint was a single press with no combo and no stretch, and a modifier's tap slot is the only place on a 36-key board that offers one for free. The full argument, the three settings that make it affordable and the two chords it costs are under stage four. New open question 14 is the one thing it left unresolved.
3. ~~**Which pair gives `?`**~~ Answered: `d`+`v`, on both boards, with layer 1's key left unbound. The question started as whether Shift plus the `d`+`r` slash combo would do the job, which was never tested and stopped mattering once the glyph got a combo of its own. Two candidates were rejected first — `a`+`x` for "tax" and "max", `s`+`c` for "scan" and "escape" — and the answer breaks the adjacency rule as squarely as they would have, on the single argument that it is `f`+`e`'s diagonal one row down. It is the weakest-defended combo in either keymap and it is tied to `d`+`r`: if that one ever misfires, this one goes with it. The reasoning, and the two things the rejections taught, are under the adjacency rule.
4. **Are position 0's return-to-base copies redundant** now that `TO(0)` is on the `t` position on layers 2 through 5? Mostly answered. The middle key's copies, on layers 3 and 4, were deleted outright and nothing missed them. Layer 1's copy has now gone too, and the reasoning that had made it the hard case turned out to be false: layer 1 has no `TO(1)`, `MO(1)`, `TG(1)` or combo entry on either board, so it is only ever held, and releasing the thumb is the way home. What is left is the copy on layers 2 to 5, which genuinely does duplicate position 5 on each of those layers. Nothing argues for keeping it; it simply has not been deleted yet. `DF(0)` versus `TO(0)` no longer matters for layer 1 and is still unsettled on the Ximi2 elsewhere.
5. ~~**What happens to `LCTL(GRAVE)`?**~~ Answered: dropped, not relocated, and no substitute. The worst case is pressing `⌃` and `` ` `` explicitly, which was never expensive enough to justify a position. Dropping it on the Corne also closed a divergence rather than opening one, since the Ximi2 had cleared that position already and the Corne was the board still binding it. Caps Lock, which this question used to be half about, is settled and has since moved off that column entirely: layer 4's right pinky bottom on both boards, inside the 36-key core, with the left thumb's dance as a second route on each.
6. **Is slash staying combo-only acceptable permanently?** It has been fine for weeks, but weeks of a combo-only glyph is not the same test as a year of it, and file paths are a thing you type.
7. **Do home row mods happen at all,** given how much better the board felt after the tap dances came out? The answer depends entirely on whether opposite-hand-only holds behave as advertised on both firmwares.
8. **Where does `M10` go?** Lock screen is the last tenant of the right outer column, and it is currently the only thing keeping that column from being entirely dead.
9. **Is combo-only Bluetooth acceptable permanently?** The same question as slash, with a worse failure mode. Slash staying combo-only costs keystrokes if it breaks; Bluetooth staying combo-only means a broken combo is answered with the physical reset button. `require-prior-idle-ms = <250>` is the only thing standing between the `o`+`p` anchor and a roll, and nothing has yet tested it against a debounce change or a worn switch.
10. **Does a 36-key Ximi build exist at all?** Unknown, and worth finding out before stage six rather than during it. If it does not, de-keying the current board is the only route for work, which changes nothing about the plan but changes what stage six costs.
11. ~~**If the current Ximi2 is kept and de-keyed rather than replaced, what happens to the extra cluster?**~~ Answered: it stays usable, as a work-board bonus. The question assumed the choice was between a usable cluster and strict parity, and that framing was wrong — the two are compatible under a tenancy rule. Nothing may have its **only** home out there, so every habit still transfers, and the cluster gets to be what it actually is, which is eight free keys on one of the two boards. The stage that existed to empty it is deleted.
12. **Why is the base-layer left thumb right Cmd?** `KC_RGUI` on the Ximi2, `&kp RGUI` on the Corne, while every other layer on both boards uses left Cmd. It was copied faithfully rather than silently corrected, which was the right instinct during a port and is not an answer. Either there is a reason nobody has written down, or it is a slip that has been load-bearing long enough to feel deliberate. Decide, then make all twelve positions agree.
13. ~~**Does the left-thumb Caps Lock dance survive?**~~ Answered, twice, and the second answer reversed part of the first. The question proposed dropping the dance so both boards could take Caps Lock off layer 4. Instead the dance grew — it carries Caps Word on a double-tap too — and layer 4's key was deleted on both boards, which left the Corne with no Caps Lock at all. That was the wrong end state and it has since been corrected: the layer 4 key is back on both boards — and has since moved off the doomed column onto layer 4's right pinky bottom, inside the core — the dance keeps its extra behaviour on the Ximi2, and the gesture is now a shortcut rather than the sole route. Then a third answer reversed the rest of the first: the Corne now carries the dance too, `shift_caps` at 200 ms on layers 0 and 1, with Caps Lock on a triple-tap because ZMK has no hold slot. The objection that had blocked it — that a double-tap would fire while typing two capitals — was simply wrong about ZMK's tap-dance, which resolves a held Shift on the following letter and not on a second tap. So the capability gap is closed on the board that had none. Worth remembering three times over — a tap-dance term can be worth paying, a capability that exists on only one board is a bug even when that board is the one you use most, and an objection that rests on how a firmware behaves is worth re-reading the firmware for before it becomes a decision.

14. **Can a Vial key override trigger on a mod-tap keycode?** This is the whole distance between the two boards on Tab. The Corne's mod-morph makes Shift+Tab a keydown event, so release order cannot spoil it. Vial's `key_override` table is the right shape and is sitting empty, but it acts on resolved keycodes, and `LALT_T(KC_TAB)` does not resolve to `KC_TAB` until release. If `trigger` can be set to the mod-tap keycode itself, then `{trigger: LALT_T(KC_TAB), replacement: KC_TAB, layers: 1, trigger_mods: MOD_LSFT, suppressed_mods: 0, options: 7|16}` is an exact stand-in — bit 16 is `ko_option_no_reregister_trigger`, without which releasing the key re-fires the mod-tap. Untested, so it is deliberately **not** written into `akiva.vil`; a bad override on the work board is a worse outcome than the thing it fixes. Until it is tested the Ximi2's rule is behavioural: release Shift last. That makes a third Corne-only reliability item, beside the `&lt` knobs and layer 5's ZMK-only keys, and open question 13's warning applies — a capability on one board only is a bug even when that board is the one you use most.
15. **Does the tap-dance Shift reach the report before the mod-morph looks for it?** Two changes landed in the same pass and this is where they meet. `alt_or_tab` chooses its branch from the modifiers held at its own keydown, and Shift is now `shift_caps`, a tap-dance that emits `&kp LSHFT` on the interrupt path. If that Shift is registered before the Tab key is evaluated, Shift+Tab takes the plain `&kp TAB` branch and everything the mod-morph was added for holds. If it is not, Shift+Tab falls through to the hold-tap and the release-order hazard is back, with the added twist that the hazard would now be invisible in the keymap — both halves look correct in isolation. This is a two-minute check on the board: hold the left thumb, tap Tab, and see whether a Claude Code session cycles the mode forwards or backwards. If it fails, the fix is to widen `alt_tab`'s `hold-trigger-key-positions` reasoning rather than the morph — or to accept a plain `&kp LSHFT` on the base layer and keep the dance on layer 1 only, since the dance is the newer of the two features and the cheaper one to narrow.

---

## The shape of the whole thing

| Stage | What it costs you | Ximi2 | Corne |
|---|---|---|---|
| 1 — Retire the right column | Almost nothing, as it turned out | **Done**, bar the lock key | **Done**, bar the same lock key |
| 2 — Bring the Corne to parity | One sitting of config work | Source of truth | **Done** — layer 5 now down to five ZMK-only keys |
| 3 — Housekeeping, both boards | Nothing. No key changes | **Done** | **Done** |
| 4 — Homes for the left outer column | Thinking, mostly | **Done** — seven tenants settled, Tab onto the Alt thumb's tap slot last | The same, on both boards at once |
| 5 — Retire the left column | The real test. Two weeks | All three positions dead on base and layer 1; a redundant `&to 0` left on layers 2–5 | The same, position for position |
| 6 — Buy the hardware | Money — possibly twice | Gated on stage 5 | Gated on stage 5 |
| Side quest — home row mods | Some misfires. Thumbs stay as fallback | Not started | Not started |

Two rules, and they are the same two the plan started with. **Never advance a stage on a schedule** — advance when the previous stage's closing line is actually true of you; stages four and five are the ones worth being slow about. And **never advance on one board alone.** That second rule was suspended for exactly one stage, knowingly, and stage two is where it came back into force — which it now has. From here the two boards move together, because a single layout on two keyboards was the whole point, and the next stage that matters is the one that costs a column off both of them.

...
On the **Ximi2**, the work keyboard, the right outer column is retired on every layer. Mouse emulation is gone from the keys — no movement, no scroll, nothing but the four pointer combos that survive because no trackpad maps them. Layer access was torn out and rebuilt as a uniform momentary/locked pair per layer, the same shape on every layer, learnable in an evening. Backtick found a permanent home on the right pinky bottom, which retired tilde in the same stroke. Two Vial bugs were found, understood and fixed. The left outer column's middle and bottom keys are both dead on every layer, which means that column is down to Tab and a return-to-base copy and nothing else.
...
columns 13
  ...
  xl1["❌"] a s d f g space h j k l bspc["⌫"] xr1["❌"]
  xl2["❌"] z x c v b space n m cma[","] dot["."] grv[" `"] xr2["❌"]
  ...
  class tab,lock doomed
  class xl1,xl2,xr1,xr2 dead
  class q,w,e,r,t,a,s,d,f,g,z,x,c,v,b,y,u,i,o,p,h,j,k,l,bspc,n,m,cma,dot,grv core
  class cmd,spc,sft,ctl,ent,alt thumb
```
...
**Status — down to one decision, on one key, on both boards. Five of the six tenants are executed or rehoused and the column is down to a single live position. This is where the open questions live.**

This is the stage the document cannot finish for you, and it has very nearly finished itself. The right column was easy because almost nothing on it was load-bearing. The left column was the opposite: it held two of the keys you press most, and none of the decisions had been made. All but one have been made since, and every one of them landed on both boards at once — which was the whole point of doing stage two first, and is the dividend it pays.

Here is everything currently living on it, across all six layers, on both boards:

| Layer | Top | Middle | Bottom |
|---|---|---|---|
| 0 — base | `⇥` | dead — Escape is `q`+`e` now | dead — backtick left in stage two |
| 1 — symbols | `DF(0)`, `&to 0` on the Corne | dead | dead — `?` unbound in this stage |
...
Read that table and the column turns out to be a single live key wearing two hats. The middle and bottom positions are `&none` on all six layers on both boards; what is left on top is Tab on the base layer and a return-to-base copy on each of the other five. `TO(0)` is already prepared on the inner-index top key — the `t` position — on layers 2, 3, 4 and 5, and that column survives, which is what let the middle key's copies on layers 3 and 4 be deleted outright rather than relocated. Layer 1 is the exception and it is the reason the top key cannot simply follow them: its `t` position is `$`, so position 0 is the only way home from the symbols layer.
...
**Tab** still has no candidate, and it is now the only real tenant of that column without one — a claim this section has made before, which Caps Lock falsified for a while and which is true again now that Caps Lock has gone into the core. Shell completion drives it, so it needs to be cheap. The symbols-layer thumb area and the left inner column are both plausible, and neither has been tried. One worry here has already been discharged: the `⇥`+`b` prose macro, which would have gone dark the moment Tab did, was re-anchored to `q`+`b` on both boards. It is inside the core, it skips three columns, and a roll cannot reach it — which is what you want of a twenty-three-character macro. What Tab has picked up instead is a second problem on the same key: with the middle position dead, position 0 is also the only way off layer 1, so whatever answers Tab has to answer the symbols layer's escape hatch too, or the hatch has to move first. Those used to be two worries on two keys and they are one key now.
...
Add those up and the stage has spent itself down to one key. Five tenants are settled: `LCTL(GRAVE)` dropped outright, `?` moved to `d`+`v`, Escape replaced by `q`+`e` rather than joined by it, Caps Lock moved into the core, and the middle key's two return-to-base copies deleted as redundant. Two positions of three are dead on all six layers on both boards, which is why stage five is already two-thirds run below. What is left is **Tab**, and Tab alone — with no candidate, and now carrying layer 1's only escape hatch on the same key, so one answer has to serve both. The closing line below is one key away from being true, and it is the key this document has had the least to say about from the beginning.
...
**Status — two of the three positions are already live-fire, on both boards, ahead of this stage's own schedule. Only the top key still pays. This is the gate.**

The real test. Bind all three positions to nothing on every layer, on both boards at once, and live there for two full weeks. Stage four did two-thirds of that without meaning to: the middle and bottom positions are `&none` on all six layers of both boards already, so the two weeks are running on them now, and the feedback this stage exists to harvest is arriving whether or not the stage has been declared open. Only position 0 is still paying, and it pays for Tab and for layer 1's way home. The instruction to delete the last combos that reference the column has already been carried out ahead of time: `⇥`+`b` was the only one, and it is re-anchored on `q`+`b`. Nothing else in either combo table touches those three positions, so the last of the column can be switched off in a single edit per board with no collateral. The picture below draws the right outer column dead too, because that is where it ends up — which assumes the lock screen has found a core home by then. It has not. Open question 8 is the whole distance between this diagram and the literal truth.
...
4. **Are position 0's five return-to-base copies redundant** now that `TO(0)` is on the `t` position on layers 2 through 5? Answered in part: the middle key's copies, on layers 3 and 4, were deleted outright and nothing missed them, which is the evidence the rest of the question wanted. What is left is the top key, where a copy sits on layers 1 through 5, and layer 1 is the hard case — its `t` position is `$`, so position 0 is the only way home from the symbols layer and cannot be deleted until something else is. `DF(0)` versus `TO(0)` is still unsettled on the Ximi2, where the Corne has already answered by fiat: the port turned every one into `&to 0` and nothing in the file could say what `DF(0)` was buying.
...
| 4 — Homes for the left outer column | Thinking, mostly | Five tenants settled; Tab alone open, carrying layer 1's hatch | The same, on both boards at once |
| 5 — Retire the left column | The real test. Two weeks | Two positions of three already dead | Two positions of three already dead |

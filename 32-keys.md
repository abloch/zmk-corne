...
What the move costs is `⌥⇥` from that key, since Alt is its own hold. That went to layer 1's right pinky home key as a precomposed `⌥⇥`, joining the `⌥`-digit window-management block — one-shot, because a precomposed keycode releases Alt with the Tab and so cannot hold a switcher open and cycle. It displaced `^`, which took the `t` position, which held a second copy of `$`; `$` keeps the home-row slot as the commoner glyph on the better finger, and a stage-three "nothing defined twice" violation closed in the same edit. `⌃⇥` is the other casualty and it is geometric rather than logical: Ctrl and Tab are now both right-thumb positions, so the chord wants one thumb on two keys. Layer 2's dead bottom-left row has six free core positions if that turns out to matter.

...
The left thumb is the one place the purge could not reach. The Ximi2's Shift there is a dance, `tap_dance[0]`, and it has since taken on both caps behaviours — tap for Shift, double-tap for Caps Word, tap-then-hold for `M11`, which is Caps Lock. ZMK can express neither of those slots: it has no tap-then-hold at all, and a double-tap-to-caps is the obvious substitute and the wrong one, because it fires the moment you type two capitals in a row. So the Corne's left thumb is a plain Shift, and Caps Word sits on layer 4's left inner thumb on both boards.

That is worth stating as a decision and not just as a limitation, because the thumb was proposed for the Corne too — put Caps Lock on the Shift key as `M11` and the left outer column loses a tenant for free. It cannot be had. The two shapes ZMK offers in place of the missing slot, a hold-tap or a tap-dance, both charge a tapping term on plain Shift, and that is precisely the bill the `gui5`/`alt5` purge tore up, levied this time on the thumb you hold most often. Paying it to rehouse a key you press a few times a week would be the worst trade in the document.

Caps Lock is where the gap used to show, and it has closed twice over. When Caps Lock moved onto the Ximi2's thumb it vacated layer 4's left outer column, and that key was deleted on both boards — which left the Corne with no Caps Lock binding at all. The Ximi2 put the key back, alongside the gesture rather than instead of it, and the Corne bound the same position; and it has now moved again, off the doomed column and onto layer 4's right pinky bottom — the key that carries backtick on the base layer — on both boards. So it is inside the 36-key core at last: two routes on the Ximi2, one on the Corne, and neither of them on a key that is going away. The asymmetry stays, and it is a capability difference rather than a divergence in the maps, which say the same thing at the same position.
...
One worry here was already discharged before the move: the `⇥`+`b` prose macro, which would have gone dark the moment Tab did, was re-anchored to `q`+`b` on both boards. Nothing else in either combo table names position 0, or any thumb, so the move broke no combo on either board.
